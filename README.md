...
因此我们可以先把数据搬运到访问延迟比较低地共享内存，后续计算内积时可以从共享内存读取数据。
...
```
这里我根据warp0的tx、ty线程索引给出了共享内存读取索引情况,先看k=0的情况，可以看到我对分了8个颜色块，这代表内存事务发送的数据申请索引，这里其实我之前
...
前面的共享内存必须等待数据读取完毕才能进行下一步操作，我们引入异步拷贝操作，直接从全局内存拷贝到共享内存，
...
```
cp.async.cg.shared.global [dst], [src], 16;   // cg: 只能 16B
cp.async.ca.shared.global [dst], [src], sz;   // ca: 4/8/16B
```
| 维度     | cg                  | ca                 |
|--------|---------------------|--------------------|
| 缓存路径	  | L2 → shared（绕过 L1）  | 	L2 → L1 → shared  |
| 大小	    | 仅16B	               | 4/8/16B            |
| 对齐要求	  | 16B 对齐	             | 按大小对齐              |
| 数据复用假设 | 不复用	                | 可能复用               |
| L1 影响  | 不污染 L1	             | 占用/污染 L1           |
| 带宽效率   | 高（128-bit 事务）	      | 取决于大小              |
| 适用场景   | 大块流式 tile 加载	       | 小粒度、不规则、有复用        |
| 典型用例   | GEMM/Attention tile | 	    转置、gather、小元素 |
